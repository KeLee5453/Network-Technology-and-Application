...
//	ON_COMMAND(IDC_RETUREN, &CAboutDlg::OnReturen)
END_MESSAGE_MAP()
...
void CARPDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LIST2, mc_list);
	DDX_Control(pDX, IDC_GET, mc_get);
	DDX_Control(pDX, IDC_RETUREN, mc_return);
}
...
BEGIN_MESSAGE_MAP(CARPDlg, CDialogEx)
	...
	ON_WM_QUERYDRAGICON()
	ON_WM_CLOSE()
	ON_NOTIFY(IPN_FIELDCHANGED, IDC_IPADDRESS1, &CARPDlg::OnFieldchangedIpaddress1)
	ON_BN_CLICKED(IDC_GET, &CARPDlg::OnClickedGet)
	ON_BN_CLICKED(IDC_RETUREN, &CARPDlg::OnClickedReturen)
END_MESSAGE_MAP()
...
void CARPDlg::OnFieldchangedIpaddress1(NMHDR* pNMHDR, LRESULT* pResult)
{
